...
| 38 | GNOME Shell: uninstall apps from the app grid | Built as a GNOMEAI app | [`Flatpak Remove`](apps/flatpak-remove/) lists user-installed Flatpak apps and requires confirmation before invoking Flatpak removal. Shell itself has no package-manager uninstall backend. |
| 39 | GTK: spellchecking | Built as a GNOMEAI app | GTK still needs a provider API for native widget integration. [`Spell Lens`](apps/spell-lens/) offers local Hunspell-backed spelling checks without sending text to a service. |
